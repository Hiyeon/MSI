    ...
    def __getitem__(self, index):
       ...
       #### get LQ images
        ...
        if self.opt['ssl'] and self.opt['use_time'] == True:
            return {'LQs': img_GTs, 'GT': img_GTs, 'key': key, 'time': time_tensor}

        elif self.opt['ssl']:
            return {'LQs': img_GTs, 'GT': img_GTs, 'key': key}
        
        elif self.opt['use_time'] == True:
            return {'LQs': img_LQs[[0, -1], :, :, :], 'GT': img_GTs[1:-1, :, :, :], 'key': key, 'time': time_tensor}
        
        else:
            return {'LQs': img_LQs[[0, -1], :, :, :], 'GT': img_GTs[1:-1, :, :, :], 'key': key}
